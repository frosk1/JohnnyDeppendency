...
//    vector<string> eager_classnames {"reduce","shift","RA","LA"};
    ...
    Multiperceptron multiperceptron(standard_classnames);
    unordered_map<string,int> feature_map;


    int max_iter = 2;

    train_model(train_all, max_iter, multiperceptron, feature_map, standard, labeled);

    // unlabeled, method knows if labeled or unlabeled from the model classes
    parse_file(dev_blind,"test_out_unlabeled.txt", multiperceptron, feature_map, standard);

   // labeled, method knows if labeled or unlabeled from the model classes
//    parse_file(dev_blind,"test_out_labeled.txt", multiperceptron, feature_map, standard);
